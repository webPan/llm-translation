  ...
  protected getMessageHandlers(): Record<string, (message: BridgeMessage) => unknown | Promise<unknown>> {
    return {
      ...
      'config.templates.import': async (message) => {
        ...
        const promptManager = getPromptManager();
        const result = await promptManager.importTemplates(json, {
      ...
      // 导出模板到文件
      'config.templates.exportFile': async () => {
        const promptManager = getPromptManager();
        const json = promptManager.exportTemplates({ includeBuiltin: false });
        ...
        return { success: true, fileName: fileUri.fsPath };
      },

      // 从文件导入模板
      'config.templates.importFile': async () => {
