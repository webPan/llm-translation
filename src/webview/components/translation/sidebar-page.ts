...
@customElement('sidebar-page')
export class SidebarPage extends BaseElement {
  ...
  private async _handleCopyTranslation(text: string) {
    console.log('[SidebarPage] _handleCopyTranslation called with text:', text.substring(0, 50));
    try {
      await navigator.clipboard.writeText(text);
      console.log('[SidebarPage] Copy successful, dispatching copySuccess event');
      this._handleAction('copySuccess', { message: '复制成功' });
    } catch (err) {
      console.error('[SidebarPage] Copy failed:', err);
