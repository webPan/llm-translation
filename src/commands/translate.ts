...
import { SimplePanelManager, SidebarPanelManager } from '../webview/controllers';
    ...
    if (!useOverlay) {
      ...
      simplePanelManager.showResult(
        ...
        result,
        {
          ...
          onExpand: () => {
            const panel = sidebarPanelManager.createOrShow(context.extensionUri, context);
...
async function translateSidebar(
  ...
  try {
    ...
    console.log('[LLM Translation] translateSidebar using prompt template:', defaultTemplateId);
